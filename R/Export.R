...
#
# This file is part of GDE2025AntiplateletAnalysis
#
...
exportAnalyses <- function(outputFolder, exportFolder) {
  ...
  cmAnalysisListFile <- system.file("settings",
                                    "cmAnalysisList.json",
                                    package = "GDE2025AntiplateletAnalysis")
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "GDE2025AntiplateletAnalysis")
  tcosOfInterest <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  pathToCsv <- system.file("Cohorts.csv", package = "GDE2025AntiplateletAnalysis")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(exposureId, ".json"), package = "GDE2025AntiplateletAnalysis")
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  createOutcomeRow <- function(outcomeId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(outcomeId, ".json"), package = "GDE2025AntiplateletAnalysis")
    definition <- readChar(cohortFileName, file.info(cohortFileName)$size)
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "GDE2025AntiplateletAnalysis")
...
exportMetadata <- function(outputFolder,
  ...
  database <- tibble(databaseId = databaseId,
                     ...
                     studyPackageVersion = utils::packageVersion("GDE2025AntiplateletAnalysis"),
                     isMetaAnalysis = 0)
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "GDE2025AntiplateletAnalysis")
